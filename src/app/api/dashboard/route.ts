...
// Import sample reviews in case database needs seeding on first load from dashboard
...
export async function GET() {
  try {
    ...
    // Run aggregations and queries in parallel for efficiency
    const [
      totalReviews,
      sentimentStats,
      categoryStats,
    ...
    sentimentStats.forEach(stat => {
      if (stat._id in sentimentCounts) {
        sentimentCounts[stat._id as keyof typeof sentimentCounts] = stat.count;
      }
    });
    ...
    categoryStats.forEach(stat => {
      if (stat._id in categoryCounts) {
        categoryCounts[stat._id as keyof typeof categoryCounts] = stat.count;
      }
    });
